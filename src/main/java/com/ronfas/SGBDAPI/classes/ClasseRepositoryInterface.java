...
//    User findTeacherForClasse(UUID uuid);
    ...
    @Query( "SELECT u FROM user u left join user_cours uc on uc.userEntity = u left join role r  on r = uc.roleEntity  where r.roleType =0"
    )
    UserEntity findTeacherForClasse(UUID uuid);
}
